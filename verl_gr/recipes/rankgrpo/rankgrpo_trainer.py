...
def _print_rankgrpo_val_generation_preview(trainer, inputs, outputs, scores, ground_truths=None) -> None:
    ...
    for idx, (inp, out, score, gt) in enumerate(preview):
        ...
        print(
            f"[val_generations][{idx}] score={score} ground_truth='{gt_text}' "
            f"input='{inp_text}' output='{out_text}'"
        )
...
def rankgrpo_validate(trainer):
    ...
    val_repeat = int(getattr(val_kwargs, "n", val_kwargs.get("n", 1)))
    replicas_slept = False
    try:
        for test_data in trainer.val_dataloader:
            ...
            try:
                ...
                # Rank-GRPO advantage is recomputed from decoded text; no actor forward needed.
                ...
                val_compute_actor_loss = trainer.config.trainer.get("val_compute_actor_loss", False)
                if val_compute_actor_loss:
                    ...
                    if eval_loss is not None and math.isfinite(trainer._as_float(eval_loss, default=float("nan"))):
                        eval_loss_values.append(
                            (float(eval_loss), int(len(rank_sums) if rank_sums is not None else reward_tensor.shape[0]))
                        )
            finally:
