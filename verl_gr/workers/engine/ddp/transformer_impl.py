...
class DDPEngine(FSDPEngine):
    ...
    # ------------------------------------------------------------------
    # Overrides — training loop with gradient accumulation
    # ------------------------------------------------------------------

    def train_batch(self, data, loss_function):
        """Training step with optional gradient accumulation.

        When ``gradient_accumulation_steps > 1``, the input batch is split
        into equal-sized chunks.  ``forward_backward_batch`` runs on each
        chunk (accumulating gradients) and ``optimizer_step()`` is called
        once at the end — matching TRL's `--gradient_accumulation_steps`.
        """
        from verl.utils.tensordict_utils import maybe_fix_3d_position_ids

        accum_steps = getattr(self.engine_config, "gradient_accumulation_steps", 1) or 1
